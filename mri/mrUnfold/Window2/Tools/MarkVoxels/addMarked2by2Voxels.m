function [mVoxelsStruct, isDone] = addMarked2by2Voxels(c,r,button,mVoxelsStruct,displayStruct,grayStruct,volStruct,h_fig2);
% Takes location from ginput, finds nearest 2x2 gray matter nodes in current slice to it, and adds it
...
if (button ~= 3)
  ...
  % 
  [v idx(1)] = min(abs(x - c) + abs(y - r));
  [v idx(2)] = min(abs(x - (c+1)) + abs(y - r));
  [v idx(3)] = min(abs(x - (c+1)) + abs(y - (r+1)));
  [v idx(4)] = min(abs(x - c) + abs(y - (r+1)));
