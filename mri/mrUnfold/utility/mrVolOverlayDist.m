function [selectedNode, showDistance, imgStruct] = ...
		    mrVolOverlayDist(volStruct,displayStruct,...
				    grayStruct,selectedNode,cutNodes,mVoxelsStruct,h_fig2) 
% 
% [grayStruct,showDistance,imgStruct] = mrVolOverlayDist(volStruct,displayStruct,...
...
%
% HISTORY: 03.18.97 ABP -- Added slice orientations
...
% BUGS:    What will this do to selectedNode when it is chosen from another routine?? (ABP)
...
if (selectedNode.index < 1)
  [selectedNode.index, imgStruct] = mrVolSelect(volStruct,displayStruct,grayStruct.nodes,h_fig2);
  if (selectedNode.index == -1)
    disp('ERROR:  Node not selected.')
    [showGray,imgStruct] = mrVolOverlayGray(volStruct,displayStruct,grayStruct.nodes(1:3,:),[],cutNodes,mVoxelsStruct,h_fig2);
    return;
  end
end
...
[showGray,imgStruct] = mrVolOverlayGray(volStruct,displayStruct,grayStruct.nodes(1:3,:),grayColorOverlay,cutNodes,mVoxelsStruct,h_fig2);
