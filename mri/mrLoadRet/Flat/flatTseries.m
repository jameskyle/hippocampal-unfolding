function tSeriesF = flatTseries(gray2flat,gray2inplane,numofanats, ...
  curSize,fSize,tFrames,exp,nExp)
...
%
% HISTORY:
%  8/23/99 SAE/MMZ Fixed bug so will average all points in 3-D space which
%       project to a single flat voxel.  The previous version would not
%       average, sort, or merge properly
%
...
%
tSeriesF = zeros(nFrames,prod(fSize));   %this is image data
%whatever grid you make count the number of voxels to grid
...
%  if xLocsFp ~= []
   if ~isempty(xLocsFp)  % hab (matlab5 update)
	...
	mrVcoord([gLocsFunc(xLocsFp,[1 2]) ...
     ...
     % SAE & MMZ -- the code immediately below does not work properly!
%    tSeriesF(:,xFlatImage) = ...
%	tSeriesF(:,xFlatImage) + tSeries(tFrames,xTseries);

     %%%SAE & MMZ --- check for duplicates and average across 'em

     nodupsxf = unique(xFlatImage);
%    disp(['Of ',num2str(length(xFlatImage)), ' points in xFlatImage', ...
%	  num2str(length(nodupsxf)), ' are unique']);

%    nodupsxt = unique(xTseries);
%    disp(['Of ',num2str(length(xTseries)), ' points in xTseries', ...
%	  num2str(length(nodupsxt)), ' are unique']);
 
%    noduppairs = unique([xTseries xFlatImage],'rows');
%    disp(['Of ',num2str(size([xTseries xFlatImage],1)),' rows of pairs', ...
%	  num2str(length(noduppairs)), ' are unique']);

% only loop for unique values, then pull duplicates from list
     numduplicates=0;
     for dupval = (nodupsxf')
     	foo = xTseries(xFlatImage == dupval);
         if(length(unique(foo))>1)
     	    numduplicates=numduplicates+1;
         	%disp(['xFlatImage value ',num2str(dupval), 'is duplicated' ...
	        %' and paired with xTseries values ']);
             %disp(num2str(foo));
     	end
     	tSeriesF(:,dupval)=tSeriesF(:,dupval)+mean(tSeries(tFrames,foo),2);

      end

     disp(['Number of duplicates: ', num2str(numduplicates)]);
 
     % Mark each grid point in the flattened image where we added a
     % a tSeries
     tSeriesC(nodupsxf)=tSeriesC(nodupsxf) + 1;
   end
end 					%for thisAnat...
