...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Testing mrLoadRet-1.0

% Create layer1map for right side of gmb brain
cd /usr/local/mri/anatomy/gmb
load UnfoldParams
dimdist = 1./volume_pix_size;
cd right
[nodes,edges,vSize] = readGrayGraph('RHGray.dat');
cd unfold
load flat
[junk1, gLocs3didx, unfList] = intersect(gLocs3d,nodes(1:3,:)','rows');
[junk2 sortList] = sort(gLocs3didx);
unfList = unfList(sortList);
layer1map = squeeze2layer1(nodes,edges,unfList,dimdist);

save flat gLocs2d gLocs3d paramsFile layer1map unfList
