...
%				'flat.mat'
%	07.07.98	bw,sjc  Added unfoldSubDir as an input parameter to
%				'mrCreateFlatAnat.m' so that one of several unfolds
%				of the same gray matter can be specified.
%				Also added a comment and unfdir to be saved out in
...
if (step(5)==1 | step(6)==1 | step(7)==1 | step(10)==1)
  ...
  if ((side == 'b') | (side == 'l'))
    ...
    leftUnfoldSubDir = input('Which unfold subdirectory?  ','s');
    
    % Check to see if the subdirectory exists
    d = sprintf('%s/%s',leftUnfDir,leftUnfoldSubDir);
    if ~(exist(d,'dir'))
  ...
  if ((side == 'b') | (side == 'r'))
    ...
    rightUnfoldSubDir = input('Which unfold subdirectory?  ','s');
    
    % Check to see if the subdirectory exists
    d = sprintf('%s/%s',rightUnfDir,rightUnfoldSubDir);
    if ~ (exist(d,'dir'))
      fprintf('No directory %s.\nSetting sub directory to null.\n',d);
      rightUnfoldSubDir = '';
    end
...
for stepnum=1:nsteps
  ...
  if (stepnum==5 & step(stepnum)==1)
    if ((side == 'b') | (side == 'l'))
      mrCreateFlatAnat(side,leftUnfoldSubDir);
    end
    if ((side == 'b') | (side == 'r'))
      mrCreateFlatAnat(side,rightUnfoldSubDir);
    end
  end
