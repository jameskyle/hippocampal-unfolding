function [showGray,imgStruct] = ...
    mrVolOverlayGray(volStruct,displayStruct,nodeXYZ,grayColorOverlay,cutNodes,mVoxelsStruct,h_fig2)
% 
%   [showGray,imgStruct] = mrVolOverlayGray(volStruct,displayStruct,nodeXYZ,grayColorOverlay,cutNodes,mVoxelsStruct,h_fig2)
% 
% AUTHOR: Wandell
% DATE: 03.09.97
% PURPOSE: 
...
if (~isempty(x) & ~isempty(y))
  ...
  if (displayStruct.flags(3) == 1)
    ...
    tmpImage = mrVolShowNodes(tmpImage,l,cutNodes,x,y,-1);
  end

  % Show marked voxels in this slice
  if (displayStruct.flags(4) == 1)

    tmpImage = mrVolShowNodes(tmpImage,l,mVoxelsStruct.V1,x,y,-2);	% V1
  
    tmpImage = mrVolShowNodes(tmpImage,l,mVoxelsStruct.V2,x,y,-3);	% V2
  
    tmpImage = mrVolShowNodes(tmpImage,l,mVoxelsStruct.V3,x,y,-4);	% V3
  
    tmpImage = mrVolShowNodes(tmpImage,l,mVoxelsStruct.V4,x,y,-5);	% V4
  
    tmpImage = mrVolShowNodes(tmpImage,l,mVoxelsStruct.MST,x,y,-6);	% MST
  
    tmpImage = mrVolShowNodes(tmpImage,l,mVoxelsStruct.Other,x,y,-7);	% Other
