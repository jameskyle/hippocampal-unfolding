...
aTheta = 0;
sagDelta = 0.003;               % angle delta in radians
sagDeltaMin = 0.003;
sagDeltaMax = 0.06;
aThetaSave = 0;
cThetaSave = 0;
...
joywin = figure('Position', [100, 100, 550, 170]); % rotation joystick win
colormap(volcmap);
...
% First we create the window that will show sagittal slices
% from the volume anatomy.  We call this window "Sag Win"
%
...
ld = uimenu('Label','Init','separator','on','backgroundcolor',[.8,.8,.8]);
...
uimenu(ld,'Label','Quit','CallBack', ...
    'delete(obwin); delete(sagwin); delete(joywin);','Separator','on');


%%%%% Sagittal File Menu %%%%%
ld = uimenu('Label','File','separator','on','backgroundcolor',[.7,.7,.7]);
...
ld = uimenu('Label','Align','separator','on','backgroundcolor',[.8,.8,.8]);

% Create of the set of inplane lines initially
uimenu(ld,'Label','Setup/Refresh Inplanes','CallBack',...
	'[obX,obY,obXM,obYM,lp,ipThickness,ipSkip] =mrSetupInplanes(numofanats,obXM,obYM,lp,volume_pix_size,inplane_pix_size,curInplane);','Separator','on');
...
%%%%% Sagittal Select Menu %%%%%

ld = uimenu('Label','Select','separator','on','backgroundcolor',[.7,.7,.7]);
...
uimenu(ld,'Label','Compute Rotation','CallBack', ...
 '[trans,rot]=mrDoAlignVol(inpts,volpts,scaleFac,curSize,sagSize,volume,numSlices,retwin,sagwin,obwin);','Separator','on');

uimenu(ld,'Label','Check Rotation','CallBack', ...
'chkImg = mrCheckAlignVol(rot,trans,scaleFac,curSize,anatmap(curSer),volume,sagSize,numSlices,obwin);','Separator','on');
...
uicontrol('style','pushbutton','string','Select Points','units','normalized',...
	...
	'[inpts,volpts]=mrSelectPoints(inpts,volpts,retwin,obwin,anatmap(curSer),obPts,obSizeOrig,reflections);');
...
uicontrol('Style','pushbutton','String','Save AlignParams','Position',[175,70,160,20],'CallBack','aThetaSave= aTheta; cThetaSave=cTheta;mrSaveAlignParams(obXM,obYM,subject,inplane_pix_size,ipThickness,ipSkip,curSag,curInplane,aTheta,cTheta)');

%uicontrol('Style','pushbutton','String','Reload AlignParams','Position',[175,40,160,20],'CallBack', 'load AlignParams;obX = obXM(curInplane,:);obY = obYM(curInplane,:);[obSize,obSizeOrig] = mrFindObSize(obX,obY,sagSize,numSlices,curInplane);[sagSlice,sagPts,obSlice,obPts]=mrRotSagVol(volume,obXM,obYM,obSize,sagSize,cTheta,aTheta,curSag,reflections,1,1/scaleFac(1,3),curInplane);lp = mrRefreshInplanes(lp,obXM,obYM,curInplane,0);');

uicontrol('Style','pushbutton','String','Reload AlignParams','Position',[175,40,160,20],'CallBack', 'load AlignParams;[obX,obY,obSize,obSizeOrig,sagPts,sagSlice,lp,obPts,obSlice]=mrReloadParams(lp,curInplane,obXM,obYM,sagSize,numSlices,volume,cTheta,aTheta,curSag,reflections,scaleFac);');

uicontrol('Style','pushbutton','String','<-Rotate','Position',[370,50,75,20],'CallBack','[obXM,obYM]=mrRotInplanes(numofanats,obXM,obYM,(-1*inpRotDelta),curInplane);[sagSlice,sagPts,obSlice,obPts]=mrRotSagVol(volume,obXM,obYM,obSize,sagSize,cTheta,aTheta,curSag,reflections,1,1/scaleFac(1,3),curInplane);lp = mrRefreshInplanes(lp,obXM,obYM,curInplane,0);');
	   
uicontrol('Style','pushbutton','String','Rotate->','Position',[450,50,75,20],'CallBack','[obXM,obYM] = mrRotInplanes(numofanats,obXM,obYM,inpRotDelta,curInplane);[sagSlice,sagPts,obSlice,obPts]=mrRotSagVol(volume,obXM,obYM,obSize,sagSize,cTheta,aTheta,curSag,reflections,1,1/scaleFac(1,3),curInplane);lp = mrRefreshInplanes(lp,obXM,obYM,curInplane,0);');

inpRotDeltaslider = uicontrol('Style','slider','Position',[397,85,100,20],'Min',inpRotDeltaMin,'Max',inpRotDeltaMax, 'Callback','inpRotDelta=get(inpRotDeltaslider,''value'');');


sagDeltaslider = uicontrol('Style','slider','Position',[120,100,40,20],'Min',...
    sagDeltaMin,'Max',sagDeltaMax, 'Callback','sagDelta=get(sagDeltaslider,''value'');');
...
if (curInplane ~=0)
	% draw the inplanes
	[obX,obY,obXM,obYM,lp,ipThickness,ipSkip] =mrSetupInplanes(numofanats,obXM,obYM,lp,volume_pix_size,inplane_pix_size,curInplane);
