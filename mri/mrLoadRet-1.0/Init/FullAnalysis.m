...
%	To be used with mrLoadRet.  Stands alone from mrLoadRet, unlike 
%	'mrFullAnalysis'
%
...
%
%	12/18/95	gmb	Wrote mrFullAnalysis.m
%	4/1/96		gmb	Created FullAnalysis.m
...
step=zeros(1,nsteps);
%disable first two steps (inital parameters, cropping)
for i=3:nsteps
	str=(['tempstr=input(',qt,inpstr(i,:),'? (y/n) ',qt,',',qt,'s',qt,');']);
	eval(str);
	if (tempstr(1)=='y')
		step(i)=1;
	end
...
for stepnum=1:nsteps
    ...
    %%%%%%%%%%%%%%%%%%%%%% Step 2, Clip and Save Anatomies  (never happens)
	...
	str=['err = unix(',qt,'cp ',sgdr,'/sg5_4_20.kk .' ,qt,');'];
	eval(str)
	...
	str=['err = unix(',qt,'/gusr3/mri/recon/garyrecon ',P1,' ',P2,qt,');'];
	eval(str);
