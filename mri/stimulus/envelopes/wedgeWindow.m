function w = wedgeWindow(angFunc,nx,ny,nt,shiftStep)
%
%	w = wedgeWindow(angFunc,nx,ny,nt,shiftStep)
%
%AUTHOR:  Wandell
%DATE:  5.27.95
%PURPOSE:
...
% DEBUG:  based on call from makeRotatingWedge
% 
% angFunc = envFunc;
% nx = 128, ny = 128, nt = envResolution/envFreq;
% shiftStep = 1;
% 
% direction = 'r';
...
for ii=1:nt
 w(:,ii) = currentAngFunc(qAng(:))';
 currentAngFunc = vecRotate(angFunc,round(ii*shiftStep)); 
 sprintf('Image %d',ii)
end

% imagesc(reshape(w(:,ii),128,128))
