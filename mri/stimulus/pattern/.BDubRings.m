...
for j=1:d
   ...
   for i=0:1
	img = scale((k)^i*checks .* window);  % img is -2, -1, 1, 2
	img(img == -2) = ones(1,sum(img == -2));
	img(img == -1) = 2*ones(1,sum(img == -1));
	img(img == 1) = 253*ones(1,sum(img == 1));
	img(img == 2) = 254*ones(1,sum(img == 2));
	img=getfix(img);
