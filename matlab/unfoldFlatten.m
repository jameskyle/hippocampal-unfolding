...
for iter=1:nIter
  ...
  for i=1:nSamp
    ...
    % squares algorithm.
    % leastsq is deprecated
    %tempLoc = leastsq('movePointErr',locs(i,:),options, ...
    tempLoc = lsqnonlin('movePointErr',locs(i,:),[], [],options, ...
...
%	'movePointGrad',oLocs,iDist,fweight,P,penalty_num,M);
    ...
    err = movePointErr(locs(i,:),oLocs,iDist,fweight,P,penalty_num);
