function luvErr = regresLuv(lTransform,xyz1,xyz2,white)
% function luvErr = regresLuv(xyz1,xyz2,white)
%
%  This is a function intended to be called by fmins()
%  in order to compute the cieluv error between two data sets.
%  The fmins() call should be structured as follows:
%   
%	fmins('regresLuv',lTransform,options,[],xyz1,xyz2,white)
%	
...
%  white is the 3 vector describing the white point for the cieluv calculation
%
...
%
  luvErr = mean(deltaEuv(xyz2,est,white));
%
%	But the mean seems ok for now.
%
% luvErr = max(deltaEab(xyz2,est,white));
% luvErr = mean(deltaEab(xyz2,est,white)) + max(deltaEab(xyz2,est,white));
%
